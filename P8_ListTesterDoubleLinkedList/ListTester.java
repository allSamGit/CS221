...
//15
	/**Scenario: [A] -> set(0,B) -> empty list
	 * @return empty list After set(0,B)
	 */
...
//48    
	...
	private IndexedUnsortedList<Integer> emptyList_addToRearA_A(){
		...
		IndexedUnsortedList<Integer> list = newList(); //We pass empty list to our list
		list.addToRear(ELEMENT_A); //changing scenario
		return list;
	...
	private IndexedUnsortedList<Integer> A_addToFrontB_BA(){
		...
		IndexedUnsortedList<Integer> list = emptyList_addToRearA_A(); //Passing addToFrontScenario(to make [A] as the list)
...
//Iterator Scenarios	
	...
	private void testSingleElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			if (SUPPORTS_LIST_ITERATOR) {
				...
				//Single Element List Iterator Test
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName +"_iterNextRemoveNext_testIterHasNext",testIterHasNext(iterAfterNext(scenario.build(),iterAfterRemove
			(iterAfterNext(scenario.build(), 1)),1),Result.True));
			printTest(scenarioName +"_iterNextRemoveNext_testIterNext",testIterNext(iterAfterNext(scenario.build(),iterAfterRemove
			(iterAfterNext(scenario.build(), 1)),1),contents[1],Result.MatchingValue));
			printTest(scenarioName +"_iterNextRemoveNext_testIterRemove",testIterRemove(iterAfterNext(scenario.build(),iterAfterRemove
			(iterAfterNext(scenario.build(), 1)),1),Result.NoException));
			
			if (SUPPORTS_LIST_ITERATOR) {
				
				//Double Element List Iterator Test
		...
		try {
			...
			//TODO: tests for scenarios resulting in a 3-element list
						...
						printTest(scenarioName + "_testListIter", testListIter(scenario.build(), Result.NoException));
					      ...
					      printTest(scenarioName + "_testListIter3.PreviousNextSet", testListIterSet(listIterAfterNext(listIterAfterPrevious(scenario.build().listIterator(3), 1), 1), ELEMENT_X, Result.NoException));
						}
		} catch (Exception e) {
			System.out.printf("***UNABLE TO RUN/COMPLETE %s***\n", scenarioName + " TESTS");
			e.printStackTrace();
	...
	private Iterator<Integer> iterAfterRemove(IndexedUnsortedList<Integer> list)
	{
		Iterator<Integer> it = list.iterator();
		it.remove();
		return it;
	 ...
	 * @return iterator
	 */
	private Iterator<Integer> iterAfterNext(IndexedUnsortedList<Integer> list,Iterator<Integer> it, int numCallsToNext)
	{
		 it=list.iterator();
		for (int i = 0; i < numCallsToNext; i++) {
			it.next();
